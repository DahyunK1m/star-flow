...
// ─── API 호출 ─────────────────────────────────
// API 엔드포인트: Netlify Function (배포) 또는 직접 (로컬 개발용)
...
const ChatScreen = ({profiles,onAddProfile}) => {
  ...
  const expertInfo = expert==="saju"
    ? {icon:"🐙",name:"문어마녀",color:C.purple}
    : {icon:"🌟",name:"불가사리",color:C.cyan};
  ...
  if(phase==="list") return (
    <div style={{padding:"20px 16px 100px"}}>
      ...
      <div style={{display:"flex",gap:10,marginBottom:20}}>
        {[
          {id:"saju",icon:"🐙",name:"문어마녀",color:C.purple},
          {id:"tarot",icon:"🌟",name:"불가사리",color:C.cyan},
        ].map(e=>(
          <button key={e.id} onClick={()=>{
            setExpert(e.id);
            setPhase(e.id==="saju"?"setup_saju":"setup_tarot");
          }} style={{
            flex:1,padding:"14px 8px",borderRadius:16,border:`1px solid ${C.border}`,
            cursor:"pointer",textAlign:"center",background:C.card,
          }}>
            <div style={{fontSize:26,marginBottom:4}}>{e.icon}</div>
            <div style={{color:C.text,fontFamily:FONT,fontSize:12}}>{e.name}</div>
            <div style={{color:e.color,fontFamily:FONT,fontSize:10,marginTop:2}}>새 상담 시작</div>
          </button>
        ))}
      </div>
          ...
          <div style={{display:"flex",flexDirection:"column",gap:8}}>
            {chatList.map(chat=>{
              const expInfo=chat.expert==="saju"
                ?{icon:"🐙",color:C.purple}:{icon:"🌟",color:C.cyan};
  ...
  return (
    <div style={{display:"flex",flexDirection:"column",height:"100vh"}}>
      ...
      <div style={{
        ...
        {/* 새 채팅 버튼 */}
        <button onClick={()=>{
          setPhase(expert==="saju"?"setup_saju":"setup_tarot");
